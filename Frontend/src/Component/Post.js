...
<User direction={"row"} id={userId} name={name || posts?.owner?.name } fs={40} image={profileImg}/>
...
<SendIcon style={{cursor:"pointer"}} onClick={()=>alert("coming soon...")}/>
...
<div id="stats" className="reactions">
   {   <span className="st-items" onClick={()=>LikesArray?.length>0 && setOpenDialog(true)} > {Likes} likes </span>}
    ...
    <ModalOverlay/>
    <ModalContent>
        <ModalHeader color={"white"} style={{display:"flex"}}>
           Liked by 
        </ModalHeader>
