...
export const getProfile = (id)=>async (dispatch)=>{
    ...
    if(data.success)
    {
        ...
        dispatch({type:"GET_PROFILE_FAIL",payload:data.message});
    }
  }
  catch(err){
...
export const followUser=(id)=>async(dispatch)=>{
    ...
    try{
        ...
        const api=`${baseUrl}/api/v1/user/follow/${id}`;
        const config={
            withCredentials:true,
            headers: {
                "content-type": "application/json"
        ...
        const {data}=await axios.get(api, config);
        if(data.success){
            dispatch({type:"FOLLOW_USER_SUCCESS",payload:data.msg});
        }
        else{
            dispatch({type:"FOLLOW_USER_FAIL",payload:data.msg});
        }
    } 
    catch(err){
        dispatch({type:"FOLLOW_USER_FAIL",payload:err.message});
    }
}
...
export const searchUser=(name) =>async(dispatch)=>{
    ...
    const {data} =await axios.get(api,config);
    if(data.success) {
dispatch({type:"SEARCH_SUCCESS", payload:data.people})
    }
    else {
        dispatch({type:"SEARCH_FAIL", payload:data.msg})
    }
}
catch (err) {
