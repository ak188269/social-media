...
const [userlogo,setUserLogo] =useState("user_logo.png");
...
useEffect(()=>{
 ...
 else dispatch({type:"REGISTER_CLEAR"});
setTimeout(()=>{
  navigate("/");
},600);
...
setUserLogo("oops.gif");
 timeout=setTimeout(() => {
  setUserLogo("user_logo.png");
}, 5000);
  }
...
user.style.transform="rotateY(360deg)";
  ...
  return (
    <>
    <ToastContainer/>
    {loading ? <Loader loadingMsg='Please wait ...'/> :
    <div id="login-system">
      <section id="side-image"><img src="login-bg.png" alt="" /></section>
    <section id="user">
      <form id="user-login" onSubmit={handleLogin}>
      <div className="main-items user-logo"><img src={userlogo} id="user_logo" alt="" />
       </div>
