...
navigate("/login");
   ...
   },[dispatch,error,user,loading,navigate]);
...
if(!pwdError && !pwdLoading && msg){
   setOpenDialog(false);
   toast(msg,{position:"top-left",type:"success",autoClose:600});
   dispatch({type:"CHANGE_PASSWORD_CLEAR"});
}
else {
   toast(pwdError,{position:"top-left",type:"error",autoClose:600});
}
  ...
  return (
   ...
   <Link to={`#`} onClick={()=>setOpenDialog(true)} className="settings-items"><KeyIcon/> <span>Change Password</span></Link>
   <Link to={`#`} 
  ...
  <div className="settings-items" style={{justifyContent:"center",border:"none",marginTop:"10px"}} onClick={handleLogout}>
    ...
    <Button color={"white"} colorScheme="green" style={{float:"right"}} onClick={()=>setOpenDialog(false)}>No</Button>
    </>
