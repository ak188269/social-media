...
const register = async (req, res) => {
  try {
    ...
    //   Note  : this ids are taken from online database and will not work for local database
      ...
      following: [
        "659a88981ddc2e8b7ac3f672",
        "659a8c721653f0766e13a3e8",
        "659a8ad81ddc2e8b7ac3f67e",
      ],
    });
    //  this will push newly created users id to the followers list of these celebs
    const celebs1 = await User.findById("659a88981ddc2e8b7ac3f672");
    const celebs2 = await User.findById("659a8c721653f0766e13a3e8");
    const celebs3 = await User.findById("659a8ad81ddc2e8b7ac3f67e");
    if (celebs1) celebs1.followers.push(user._id);
    if (celebs2) celebs2.followers.push(user._id);
    if (celebs3) celebs3.followers.push(user._id);
...
const login = async (req, res) => {
  try {
    ...
    if (req.isVerified === true)
      return res.send({
        success: true,
        msg: "Logged In successfully using jwt✔",
        user: req.user,
      });
    else {
      ...
      const token = jwt.sign(payload, secretKey);
      res.cookie("jwt", token, { maxAge: expiryTime, httpOnly: true });
      user = await User.findById(user._id).populate("following");
      res
        .status(200)
...
const updateProfile = async (req, res) => {
  try {
    ...
    if (imageUrl) {
      ...
      const myCloud = await cloudinary.uploader.upload(imageUrl, {
        folder: "avatars",
      });
      // Generate a URL for the cropped image
      const croppedImageUrl = cloudinary.url(myCloud.public_id, {
        aspect_ratio: "1:1",
        background: "#262c35",
        border: "5px_solid_rgb:ffffff",
        gravity: "auto",
        radius: "max",
        width: 1000,
        crop: "fill",
...
const followUser = async (req, res) => {
  try {
    ...
    if (loggedInUser.following.includes(req.params.id)) {
      ...
      return res
        .status(200)
        .json({ success: true, msg: "You unfollowed " + userToFollow.name });
    }
...
const myProfile = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, msg: "fetched successfully", data: profile });
  } catch (err) {
    return res.json({ success: false, msg: err.message });
  }
};
...
const getAllUsers = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, msg: "fetched successfully", data: users });
  } catch (err) {
    return res.json({ success: false, msg: err.message });
  }
};
